...
class QuickLoadTestEndpointTest {

    @Mock
    private PlayerService playerService;

    @InjectMocks
    private QuickLoadTestEndpoint quickLoadTestEndpoint;

    @Test
    void quickTest_withSuccessfulRequests_returnsCompletedStatus() {
        // Arrange
        when(playerService.getPlayerInfoForLoadTest("Messi"))
                .thenReturn(Map.of("status", "SUCCESS_FROM_DB"));

        // Act
        Map<String, Object> result = quickLoadTestEndpoint.quickTest();

        // Assert
        assertEquals("COMPLETED", result.get("finalStatus"));
        assertEquals(6, result.get("successfulRequests")); // 2 connections * 3 requests
        assertEquals(0, result.get("failedRequests"));
        assertEquals(100.0, result.get("successRate"));
        assertEquals("quick", result.get("testType"));
    }

    @Test
    void quickTest_withSomeFailedRequests_returnsCorrectCounts() {
        // Arrange
        when(playerService.getPlayerInfoForLoadTest("Messi"))
        ...
        // Assert
        assertEquals("COMPLETED", result.get("finalStatus"));
        assertEquals(5, result.get("successfulRequests"));
        assertEquals(1, result.get("failedRequests"));
    }

    @Test
    void quickTest_withServiceException_returnsCorrectCounts() {
        // Arrange
        when(playerService.getPlayerInfoForLoadTest("Messi"))
    ...
    @Test
    void quickTest_whenTestIsAlreadyRunning_returnsError() {
        // Arrange
        ...
        Thread testThread = new Thread(() -> quickLoadTestEndpoint.quickTest());
        testThread.start();
